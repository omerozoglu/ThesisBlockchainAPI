...
function verifyMerkleProof(leaves, root) {
  ...
  for (let i = 1; i < leaves.length; i++) {
    genHash1 =
      i % 2 === 0 ? SHA256(genHash1 + leaves[i]) : SHA256(leaves[i] + genHash1);
    genHash2 =
      i % 2 === 0 ? SHA256(leaves[i] + genHash2) : SHA256(genHash2 + leaves[i]);
  }
...
function buildMerkleTree(transactions) {
  ...
  let hashList = transactions.map((transaction) =>
    Node(SHA256(JSON.stringify(transaction)))
  );
